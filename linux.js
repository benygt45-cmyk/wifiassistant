...
${C.x}`);
  console.log(`${C.g}wifiassistant | Linux | by benygt45${C.x}\n`);
...
const disconnectWifi = () => cmd("nmcli device disconnect wlan0 2>/dev/null");
...
// ---------- REPORTS ----------
function jsonReport() {
  const file = path.join(os.homedir(), "wifiassistant_report_linux.json");
  const data = {
    user: "benygt45",
    ip: ipInfo(),
    wifi: activeWifi(),
...
function txtReport() {
  const file = path.join(os.homedir(), "wifiassistant_report_linux.txt");
...
function menu() {
  ...
  rl.question("> ", c => {
    ...
    if(c==="18"){ console.log("Linux uyumlu sürüm, root gerektiren işlemler için uyarı veriyor."); return pause(); }
